...
outputDirectory="./reconit_results"
########################################
# Happy Hunting
########################################
...
cat $outputDirectory/$domain/$foldername/urllist.txt | xargs -P$subdomainThreads -I % sh -c "python3 $HOME/tools/dirsearch/dirsearch.py -e $dirsearchExtensions -w $dirsearchWordlist -t $dirsearchThreads -u % | grep Target && tput sgr0 && bash ./reconit.sh -r $outputDirectory -r $domain -r $foldername -r %"
}
